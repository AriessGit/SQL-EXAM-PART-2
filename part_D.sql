...
/* 1. Implement Many-to-Many Relationship:
	o In the tourist question, modify the CREATE TABLE for tourists and remove the 
		foreign key (FK) of the trip.
	o Create a new table for trip_id and tourist_id.
	o Use INSERT statements to assign multiple trips to multiple tourists. */
...
/*	2. Create the Movies Table in PostgreSQL
	Write a Stored Procedure (SP) that accepts a year and returns the total revenue 
	of all movies from that year.
	o Call the SP using SELECT*/
...
DROP PROCEDURE IF EXISTS tot_rev_year();

CREATE OR REPLACE PROCEDURE tot_rev_year(
year_choice INT,
OUT total_revenue REAL)
LANGUAGE plpgsql AS 
$$
BEGIN
...
CALL tot_rev_year(2022,total_revenue => NULL);
